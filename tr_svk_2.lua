...
function Tr_svk2()
	...
	-- A.nvim_buf_set_name(bufnr, "tr_svk_2.txt") -- Assign a name to the new buffer

	local t = os.date("*t")

	-- vim.cmd("e temp")
	BName = "tr_svk_2_" .. t.hour .. "H_" .. t.min .. "M_" .. t.sec .. "S.txt"
	vim.cmd.edit(BName)
	...
	for row1 = 1, linesNum do -- The range includes both ends.
		...
		if string.match(line1, "^SR_") then -- `Live`
			...
			if iBrojac == 1 then
				svk_niti1 = "(niti " .. nit[1] .. ", " .. (nit[1] + 1) .. " FC/PC)" -- substring from position 2 to end of secondWord
				svk_niti1LC = "(niti " .. nit[1] .. ", " .. (nit[1] + 1) .. " LC/PC)" -- substring from position 2 to end of secondWord
			end
		end
	...
	svk2 = "SVK " .. SVKbroj2 -- mlabrkic

	svk_niti2 = "(niti " .. nit[#nit] .. ", " .. (nit[#nit] + 1) .. " FC/PC)" -- substring from position 2 to end of secondWord
	svk_niti2LC = "(niti " .. nit[#nit] .. ", " .. (nit[#nit] + 1) .. " LC/PC)" -- substring from position 2 to end of secondWord
	...
	-- if string.gmatch(sr1, string.sub(svk_lok[1], 4)) then
	--   sr1 = string.gsub(sr1, string.sub(svk_lok[1], 4), "")
	-- else
	--   sr1 = "_"
	-- end
	...
	-- if string.gmatch(sr2, string.sub(svk_lok[#svk_lok], 4)) then
	--   sr2 = string.gsub(sr2, string.sub(svk_lok[#svk_lok], 4), "")
	-- else
	--   sr2 = "_"
	-- end
